...
int main(int argc, char **argv){
    ...
    while (line_number){
        ...
        if (line_number > lines_count){
            fprintf(stderr, "You try get %d line but file contains only %d lines\n", line_number, lines_count);
            close(fd);
            return 3;
        }
        if (print_line_by_number(file_table, line_number, file) == -1){
	        fprintf(stderr, "Cant move cursor in file\n");
		    return 4;
        }
    }
    close(fd);
